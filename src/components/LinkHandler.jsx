...
const navigation=useNavigation()
  ...
  const makePhoneCall = () => {
    const phoneNumber = data.phone; // Assuming you pass the phone number as a prop
...
const renderItem2=({item})=>{
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`https://mangaautomobiles.com/api/getcardetail/${route.params.id}`, {
        method: 'GET'
      });
      ...
      setData(dat); // Set the data using setItm
